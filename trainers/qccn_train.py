...
from utils.util import mixup_data,mixup_data_r18
...
def default_train(train_loader,model,optimizer,writer,iter_counter):
    way = model.way
    resnet12 = model.resnet
    query_shot = model.shots[-1]
    train_shot = model.shots[0]
    target = torch.LongTensor([i//query_shot for i in range(query_shot*way)]).cuda()
    target_support = torch.LongTensor([i//train_shot for i in range(train_shot*way)]).cuda()
    ...
    for i, (inp, _) in enumerate(train_loader):
        ...
        inp = inp.cuda()
        if resnet12 ==True:
            support_da = mixup_data(inp, way, train_shot, query_shot, alpha=1.0)
        else:
            support_da = mixup_data_r18(inp, way, train_shot, query_shot, alpha=1.0)

        log_prediction, log_prediction_s, s = model(inp, support_da)
        frn_loss = criterion(log_prediction, target)
        frn_loss_2 = criterion(log_prediction_s, target_support)
        aux_loss = auxrank(s)

        loss = frn_loss + aux_loss + frn_loss_2
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
